    ...
    with col_left:
        ...
        with btn_col:
            if st.button(
                ...
                key="load_demo_data_btn",
                width="stretch",
                help="Instantly load sample datasets demonstrating NEW, CHANGED, UNCHANGED, and DELETED records without needing local CSVs.",
            ):
        ...
        elif st.session_state.get("use_demo_data"):
            from src.scd2_copilot.config import SAMPLE_DATA_DIR
            demo_source = SAMPLE_DATA_DIR / "source_today.csv"
            demo_target = SAMPLE_DATA_DIR / "target_yesterday.csv"
            if demo_source.exists() and demo_target.exists():
                source_file = demo_source
                target_file = demo_target
                st.info("Loaded built-in demo dataset (source_today.csv & target_yesterday.csv).")
