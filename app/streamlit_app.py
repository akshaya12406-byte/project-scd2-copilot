...
# Add project root and app dir to path so src and app packages are reliably importable
...
if source_file and target_file:
    ...
    # No files uploaded and no demo data loaded
    st.markdown(
        f"""
        <div class="empty-state" style="margin-top:30px;">
            <div class="empty-icon">{_icon("folder")}</div>
            <div class="empty-title" style="font-weight:600; font-size:1.1rem; margin-top:8px;">Ready to Build SCD Type 2 Tables</div>
            <div class="empty-text" style="color:var(--text-2); font-size:0.9rem; max-width:560px; margin: 8px auto;">
                Upload your <strong>Source CSV</strong> (today's data) and <strong>Target CSV</strong> (yesterday's SCD2 table) above, or click <strong>Load Built-in Demo Data</strong> for an immediate one-click pipeline run.
            </div>
        </div>
        """,
